...
const ProfilePage: React.FC = () => {
  ...
  const handleReviewerApplication = async (email: string) => {
    try {
      ...
      try {
        await logAction('reviewer_application_submitted', userData?.id || user?.id, { email });
        // notify admins (null user_id used for broadcast to admin UI workers)
        await createNotification(null, 'New reviewer application', `User ${(userData?.id || user?.id)} applied as reviewer with ${email}`);
        // notify applicant
        await createNotification(userData?.id || user?.id, 'Reviewer application received', 'Your application is under review by admins.');
      } catch (e) {
        console.warn('Post-application hooks failed', e);
      }
    } catch (err) {
      toast.error('Failed to submit application. Please try again.');
    }
  ...
  const getInitials = (name?: string, fallback?: string) => {
    const source = name || fallback || user?.fullName || userData?.full_name || user?.name || '';
  ...
  const [collaborationsCount, setCollaborationsCount] = useState<number>(0);
  ...
  const loadUserProfile = async () => {
    try {
      ...
      if (userProfile) {
        ...
        // Load profile counts (ideas, followers, following, collaborations)
        try {
          await loadProfileCounts(userProfile?.id || user?.id);
        } catch (e) {
      ...
      // Load profile counts if not already loaded
      try {
        await loadProfileCounts(userProfile?.id || user?.id);
      } catch (e) {
  ...
  // Load counts for ideas, followers, following, collaborations
  const loadProfileCounts = async (profileId?: string) => {
    const id = profileId || userData?.id || user?.id;
    if (!id) return;
    try {
      // Ideas count
      const { count: ideasCnt } = await supabase
        .from('ideas')
        .select('*', { count: 'exact', head: true })
        .eq('author_id', id);
      setIdeasCount(ideasCnt || 0);
      ...
      // Following count (who this user follows)
      ...
      // Collaborations: count of active project_collaborators entries for this user
      const { count: collabCnt } = await supabase
        .from('project_collaborators')
        .select('*', { count: 'exact', head: true })
        .eq('user_id', id)
        .eq('status', 'active');
      setCollaborationsCount(collabCnt || 0);
    } catch (err) {
      console.warn('Error loading profile counts:', err);
    }
  };
  ...
  const handleAvatarUpload = async (file: File) => {
    ...
    if (!id) {
      toast.error('Unable to identify user for avatar upload');
      return;
    }
    ...
    try {
      ...
      const filePath = `avatars/${id}-${Date.now()}.${ext || 'png'}`;
      ...
      // Get public URL
      const { data: publicData } = supabase.storage.from(STORAGE_BUCKETS.AVATARS).getPublicUrl(filePath);
      ...
      const { error: saveErr } = await supabase.from('users').update({ avatar_url: publicUrl }).eq('id', id);
      toast.dismiss();
      if (saveErr) {
      ...
      setUserData((p: any) => ({ ...p, avatar_url: publicUrl }));
      try {
    ...
    <div className="min-h-screen bg-gradient-to-br from-secondary-900 via-secondary-800 to-secondary-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="relative -mt-20 flex flex-col md:flex-row md:items-end md:space-x-8">
              ...
              {showAvatar && (
                <div className="relative mb-4 md:mb-0">
                  {(!userData?.avatar_url || avatarBroken) ? (
                    <div className="w-32 h-32 rounded-2xl border-4 border-white shadow-xl bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center text-white text-2xl font-bold">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="text-lg text-secondary-300">@{userData?.username || user?.username || userData?.email?.split('@')[0]}</p>
                    </div>
                    <div className="mt-4 sm:mt-0 flex space-x-3">
                      ...
                      {/* Apply for Reviewer button (show if not reviewer) */}
                      {userData && userData.reviewer !== true && !reviewerRequested && (
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                        ...
                        >
                          ...
                          <span>Apply for Reviewer</span>
                        </motion.button>
                      )}
                      {reviewerRequested && (
                        <span className="text-green-500 font-semibold ml-2">Application Pending</span>
                      )}
                      {userData && userData.reviewer === true && (
                        <span className="text-green-500 font-semibold ml-2">Reviewer</span>
                      )}
                    </div>
                  </div>
        ...
        >
            ...
            { label: 'Ideas', value: ideasCount, icon: FireIcon, color: 'text-orange-400' },
            { label: 'Upvotes', value: userProjects.reduce((sum, p) => sum + (p.like_count || 0), 0), icon: HeartIcon, color: 'text-red-400' },
            { label: 'Followers', value: followersCount, icon: UsersIcon, color: 'text-green-400' },
            { label: 'Following', value: followingCount, icon: UserCircleIcon, color: 'text-purple-400' },
            ...
            { label: 'Collaborations', value: collaborationsCount, icon: UsersIcon, color: 'text-pink-400' }
          ].map((stat) => (
            <motion.div
              key={stat.label}
              whileHover={{ scale: 1.05 }}
                  ...
                  <div className="glass rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                      {userProjects.slice(0, 3).map((project) => (
                        ...
                        >
                          <img
                            src={project.image || 'https://images.unsplash.com/photo-1555949963-aa79dcee981c?w=300&h=200&fit=crop'}
                            alt={project.title}
