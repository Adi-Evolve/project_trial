...
contract ZKProofVerifier is Ownable {
    ...
    /**
     * @dev Get verifying key components (since struct getter is not available)
     */
    ...
    function getVerifyingKeyIC(uint256 index) external view returns (uint256[] memory) {
        require(index < verifyingKey.ic.length, "Index out of bounds");
        return verifyingKey.ic[index];
    }
    
    function getVerifyingKeyICLength() external view returns (uint256) {
        return verifyingKey.ic.length;
    }
